...
# operator that calls the function to import the structure from the PDB
# class MOL_OT_Import_Protein_AF_List(bpy.types.Operator):
#     bl_idname = "mol.import_protein_af_list"
#     bl_label = "import_protein_fetch_af_list"
#     bl_description = "Download and open a a list of structures from AlphaFold2 DB"
#     bl_options = {"REGISTER", "UNDO"}
#
#     @classmethod
#     def poll(cls, context):
...
#
#     def execute(self, context):
#         list_file = bpy.context.scene.mol_af_id_list_file
#
#         mol_object = load.molecule_af(
#             af_code=list_file,
#             center_molecule=bpy.context.scene.mol_import_center,
#             del_solvent=bpy.context.scene.mol_import_del_solvent,
#             include_bonds=bpy.context.scene.mol_import_include_bonds,
...
#
#         bpy.context.view_layer.objects.active = mol_object
#         self.report({'INFO'}, message=f"Imported '{list_file}' as {mol_object.name}")
#
#         return {"FINISHED"}
#
#     def invoke(self, context, event):
...
class MOL_OT_Import_Protein_AF_List(bpy.types.Operator):
    bl_idname = "mol.import_protein_af_list"
    bl_label = "import_protein_af_list"
    bl_description = "Download and open a a list of structures from AlphaFold2 DB"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        mol_objects = load.molecules_af_list(
            list_file=list_file,
        ...
        # bpy.context.view_layer.objects.active = mol_objects
        self.report({'INFO'}, message=f"Imported '{list_file}'")
...
def MOL_PT_panel_list(layout_function, ):
    # col_main = layout_function.column(heading = '', align = False)
    # col_main.alert = False
    # col_main.enabled = True
    # col_main.active = True
    ...
    # col_main.scale_y = 1.0
    # col_main.alignment = 'Expand'.upper()
    # col_main.label(text = "Download from AF")
    # row_import = col_main.row()
    # # row_import.prop(bpy.context.scene, 'mol_af_id_list_file', text='AF ID list')
    # # row_import.operator('mol.import_protein_af_list', text='Download List', icon='IMPORT')
    # row_name.prop(bpy.context.scene, 'mol_import_local_name',
    #                 text = "Name", icon_value = 0, emboss = True)
    # row_name.operator('mol.import_protein_af_list', text = "Load",
    #                     icon='FILE_TICK', emboss = True)
    # row_import = col_main.row()
    # row_import.prop(
    #     bpy.context.scene, 'mol_af_id_list_file',
    #     text = "File path",
    #     icon_value = 0,
    #     emboss = True
    # )
...
def MOL_PT_panel_ui(layout_function, scene):
    ...
    MOL_change_import_interface(row, 'AF List',    5, 108)
